...
function parseServerTime(value: string) { return /(?:[zZ]$|[+-]\d{2}:?\d{2}$)/.test(value) ? new Date(value) : new Date(`${value}Z`) }
function formatTime(value: string) { return parseServerTime(value).toLocaleString([], { timeZone: JAKARTA }) }
function toJakartaInput(value: string) { const parts = new Intl.DateTimeFormat('en-CA', { timeZone: JAKARTA, year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hourCycle: 'h23' }).formatToParts(parseServerTime(value)); const pick = (type: string) => parts.find((part) => part.type === type)?.value ?? ''; return `${pick('year')}-${pick('month')}-${pick('day')}T${pick('hour')}:${pick('minute')}` }
function fromJakartaInput(value: string) { return new Date(`${value}:00+07:00`).toISOString() }
