...
const requiredByOperation: Record<string, string[]> = {
  send_email: ['to', 'subject', 'body'], read_email: ['email_id'],
...
export function EmailComposer(props: ComposerProps) { return <>{props.operation === 'send_email' && <>{field('To', 'to', props)}{field('Subject', 'subject', props)}</>}{props.operation === 'read_email' && field('Email ID', 'email_id', props)}{props.operation !== 'read_email' && field('Body', 'body', props, true)}</> }
