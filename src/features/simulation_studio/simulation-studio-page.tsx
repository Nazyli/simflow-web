...
export function SimulationStudioPage() {
  ...
  return <main className="studio"><header><div><h1>Simulation Studio</h1><p>Configure master data, workflow versions, nodes, and transitions.</p></div><Link to="/simulation">Open Simulation Runner</Link></header>
    <section className="studio-grid"><aside><h2>Master data</h2><select value={resource} onChange={(event) => setResource(event.target.value as MasterResource)}>{masterResources.map((item) => <option key={item}>{item}</option>)}</select><button onClick={() => setSelectedRecord(null)}>Create {resource.slice(0, -1)}</button>{masterData.isPending && <LoadingState />}{masterData.data?.map((record, index) => <button className="workflow-item" key={index} onClick={() => setSelectedRecord(record)}>{String(record[Object.keys(record)[0]])}</button>)}<MasterDataForm resource={resource} record={selectedRecord} isSaving={createRecord.isPending || updateRecord.isPending || removeRecord.isPending} onSave={(id, values) => id ? updateRecord.mutate({ id, values }) : createRecord.mutate(values)} onDelete={(id) => removeRecord.mutate(id)} /><h2>Workflows</h2>{workflows.isPending && <LoadingState />}{workflows.data?.map((workflow) => <button className="workflow-item" key={workflow.workflow_id} onClick={() => { setSelectedWorkflow(workflow); setVersionId(null) }}>{workflow.workflow_name}<small>{workflow.status}</small></button>)}</aside>
