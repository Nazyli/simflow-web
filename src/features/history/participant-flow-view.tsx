...
function dagLayout(apiNodes: ApiNode[], apiEdges: ApiEdge[]): Map<string, { x: number; y: number }> {
  const nodeIds = new Set(apiNodes.map((n) => n.node_id))
  const graph = new dagre.graphlib.Graph()
  graph.setGraph({ rankdir: 'LR', nodesep: 130, ranksep: 200, marginx: 60, marginy: 60 })
...
export function ParticipantFlowView({ open, onClose, versionId, executionId, title, currentState }: { open: boolean; onClose: () => void; versionId: string; executionId: string; title: string; currentState: string | null }) {
  ...
  const view = useMemo(() => {
    ...
    const definitions = new Map((nodeCatalog.data?.nodes ?? []).map((definition: NodeDefinition) => [definition.node_type, definition]))
    ...
    const flowNodes: Node[] = apiNodes.map((node) => {
      ...
      return {
        id: node.node_id,
        type: 'workflow',
        position: node.position_x !== null && node.position_y !== null ? { x: node.position_x, y: node.position_y } : (layout.get(node.node_id) ?? { x: 80, y: 80 }),
        className: visited ? `history-node-visited${node.node_id === currentState ? ' history-node-current' : ''}` : 'history-node-unvisited',
        data: { label: node.node_name, nodeType: node.node_type, color: definition?.color ?? '#64748b', inputPorts: node.input_ports, outputPorts: node.output_ports },
      }
    })
    ...
    const flowEdges: Edge[] = apiEdges.map((edge) => {
      ...
      const sourcePort = nodeById.get(edge.source_node_id)?.output_ports.find((port) => port.id === edge.source_port_id)
      return {
        id: edge.edge_id,
        type: 'workflow',
        ...
        animated: taken,
        data: { priority: edge.priority, label: sourcePort?.label ?? edge.source_port_id, style: { color, line_style: taken ? 'solid' : 'dashed', animated: taken } },
      }
    })
      ...
      <DialogContent showCloseButton={false} className="flex max-h-[calc(100vh-48px)] w-[min(1120px,calc(100vw-32px))] flex-col gap-0 overflow-hidden p-0 sm:max-w-none">
        <div className="flex shrink-0 items-center justify-between gap-4 border-b border-slate-100 px-5 py-4">
          ...
          <div className="flex shrink-0 flex-wrap items-center gap-x-5 gap-y-2 border-b border-slate-100 px-5 py-2.5 text-[11px] font-medium text-slate-500">
            ...
            <span className="flex items-center gap-1.5"><i className="history-legend-line" />Workflow definition</span>
            <span className="flex items-center gap-1.5"><i className="history-legend-node" />Visited node ({view.visitedCount})</span>
            {currentState && <span className="ml-auto flex items-center gap-1.5 text-[#5b46c5]">● Current state: {currentState}</span>}
          </div>
              ...
              {pending ? <LoadingState /> : view.flowNodes.length === 0 ? (
                ...
                <div className="history-flow-canvas">
                  ...
                  >
                    ...
                    <Controls showInteractive={false} className="bg-white border-slate-200 text-slate-700 fill-current shadow-md" />
                  </ReactFlow>
                </div>
              )}
            </div>
