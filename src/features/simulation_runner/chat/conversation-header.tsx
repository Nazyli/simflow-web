  ...
  if (!conversation) {
    return (
      <div className="flex items-center gap-3 border-b border-slate-200 bg-white px-4 py-3">
        <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-slate-200 text-xs font-bold text-slate-500">?</span>
  ...
  return (
    <div className="flex items-center gap-3 border-b border-slate-200 bg-white px-4 py-3">
      ...
      <div className="min-w-0">
        ...
        <p className="truncate text-xs text-slate-500">{count} message{count === 1 ? '' : 's'} · {conversation.actor}</p>
      </div>
    </div>
  )
}
