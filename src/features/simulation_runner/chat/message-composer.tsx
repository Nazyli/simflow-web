...
export function MessageComposer({ target, disabled, onSubmit }: MessageComposerProps) {
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="flex min-w-0 flex-col gap-2 border-t border-slate-200 bg-white p-3"
    >
      ...
      <div className="flex items-end gap-2">
        ...
        <button
          type="submit"
          disabled={!canSend}
          className="!m-0 !inline-flex shrink-0 items-center gap-1.5 rounded-md !border-0 !bg-[#9929EA] !px-3.5 !py-2 text-sm font-semibold !text-white shadow-sm transition hover:!bg-[#7d1fc2] disabled:opacity-50"
        >
          <Send size={15} />
          Send
        </button>
