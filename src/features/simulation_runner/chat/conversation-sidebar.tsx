...
export function ConversationSidebar({ actors, selectedActor, onSelect }: ConversationSidebarProps) {
  ...
  const filteredActors = useMemo(() => {
    ...
    return query ? actors.filter((actor) => `${actor.actorName} ${actor.actorId}`.toLowerCase().includes(query)) : actors
  }, [actors, search])
  return (
    <aside className="flex shrink-0 gap-1 overflow-x-auto border-b border-slate-200 bg-white p-2 lg:w-[280px] lg:flex-col lg:overflow-y-auto lg:border-b-0 lg:border-r lg:p-2">
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="flex items-baseline justify-between gap-2">
                ...
                <span className="shrink-0 rounded-full bg-violet-100 px-1.5 text-[10px] font-semibold text-[#5b46c5]">{actor.actorId}</span>
              </span>
              <span className="flex items-center justify-between gap-2">
                <span className="truncate text-xs text-slate-500">{actor.actorId}</span>
                {actor.unreadCount > 0 ? (
                  <span aria-label={`${actor.unreadCount} unread message${actor.unreadCount === 1 ? '' : 's'}`} className="grid size-5 shrink-0 place-items-center rounded-full bg-violet-600 text-[10px] font-bold text-white">{actor.unreadCount}</span>
                ) : null}
              </span>
            </span>
          </button>
      ...
      {filteredActors.length === 0 && (
        <div className="flex flex-col items-center gap-2 px-3 py-8 text-center">
          ...
          <p className="text-xs text-slate-400">{actors.length ? 'No matching conversations.' : 'No conversations yet.'}</p>
        </div>
      )}
    </aside>
  )
